...
Image processing based on NanoImagingPack (branch feature2-calreadnoise):
...
Converted into a Napari Plugin by Benedict Diederich, benedictdied@gmail.com
"""
...
def process(exportpath=None):
    ...
    for box in (bg_box, fg_box):
        ...
        clip_var_str = clip_var.get().strip()
        if clip_var_str == "":
            clip_val = None
        else:
            clip_val = int(clip_var_str)
        box.stack = np.clip(box.stack, 0, clip_val)        
        
        lbs = lower_bound_var.get().strip()
        ubs = upper_bound_var.get().strip()
        ...
        for var in [lower_bound_var, upper_bound_var]:
            ...
            try:
                ...
                tkinter.messagebox.showinfo("Range Error", err_msg)
                print(err_msg)
                return
    ...
    try:
        (bg_total_mean, gain, Readnoise, mean_el_per_exposure, validmap, figures, Text) = nip.cal_readnoise(
            fg_box.stack, bg_box.stack, validRange=validRange, exportpath=exportpath, exportFormat=exportFormat,
            brightness_blurring=scmos_filter_var.get(), plotHist=showHist_var.get())
    except AssertionError as err:
        err_msg = err
        tkinter.messagebox.showinfo("Failed analysis", err_msg)
        print(err_msg)
        return
    ...
    if (xchunks != 1) or (ychunks != 1):
        ...
        # make any odd chunk number possible
        y_slice = slice(None, fg.shape[-2] - fg.shape[-2]%ychunks)
        x_slice = slice(None, fg.shape[-1] - fg.shape[-1]%xchunks)
        fg = fg[:, y_slice, x_slice]
        bg = bg[:, y_slice, x_slice]
        print(fg.shape)

        exportpath_parent = exportpath
        for ix, (fgxc, bgxc) in enumerate(zip(np.split(fg, xchunks, axis=-1), np.split(bg, xchunks, axis=-1))):
            print(fgxc.shape)
            for iy, (fgyc, bgyc) in enumerate(zip(np.split(fgxc, ychunks, axis=-2), np.split(bgxc, ychunks, axis=-2))):
                ...
                if exportpath_parent is not None:
                    ...
                    exportpath = None
                (bg_total_mean, gain, Readnoise, mean_el_per_exposure, validmap, figures, Text) = nip.cal_readnoise(
                    fgyc, bgyc, validRange=validRange, exportpath=exportpath, exportFormat=exportFormat,
                    brightness_blurring=scmos_filter_var.get(), plotHist=showHist_var.get())
...
advanced_frame.grid(row=4, column=0, columnspan=2, pady=5, sticky="we")

# ------------------------------------------------------------------
# Button_frame
# ------------------------------------------------------------------
scmos_filter_var = IntVar()
...
Label(button_frame, text='Fit from').pack(
                    side=LEFT, padx=5)
lower_bound_var = StringVar()
Entry(button_frame, textvariable=lower_bound_var, width=5).pack(
                    side=LEFT, padx=5)

Label(button_frame, text='to').pack(
                    side=LEFT, padx=5)  
upper_bound_var = StringVar()                                      
Entry(button_frame, textvariable=upper_bound_var, width=5).pack(
                    side=LEFT, padx=5)
...
# Advanced_frame
# ------------------------------------------------------------------
Label(advanced_frame, text='Clip at').pack(
                    side=LEFT, padx=5)
clip_var = StringVar()
Entry(advanced_frame, textvariable=clip_var, width=5).pack(
                    side=LEFT, padx=5)
